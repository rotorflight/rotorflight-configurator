...
STWinUSBDriverTaskDesc=安装 STM32 BOOTLOADER WinUSB 驱动程序（固件刷写所必需）
LicenseAgreement=许可证协议
LicenseAgreementLabel=请仔细阅读以下许可证协议。
LicenseAccepted=我 &接受协议
LicenseNotAccepted=我 &不接受协议
LicenseLabel3=请仔细阅读以下许可证协议。您必须接受本协议的条款才能继续安装。
CannotContinue=安装程序无法继续。请单击取消退出。
UninstallError=卸载配置器时出错 %1。
