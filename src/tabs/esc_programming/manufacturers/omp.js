// Ported from C:\GitHub\wingflight-lua-ethos-suite\src\wfsuite\tasks\scheduler\msp\api\ESC_PARAMETERS_OMP.lua
// and escmfg/omp/{init.lua,pages/*.lua}. Group B (native MSP, no target-switch handshake).
//
...
// Ported from escmfg/omp/init.lua's getEscModel/getEscFirmware: the "model" byte is actually
...
function describeEsc(values) {
    ...
    const parts = ["OMP", capacity];
    if (values.esc_model != null) parts.push(`SW${values.esc_model >> 4}.${values.esc_model & 0xF}`);
    return parts.join(" ");
}

// Verbatim from ESC_PARAMETERS_OMP.lua's SIM_RESPONSE, used by virtual_fc.js for dev/testing.
const simResponse = [
    208, 0, 23, 3,
    0, 0, // governor
    0, 0, // cell_cutoff
