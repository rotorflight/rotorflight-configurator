<script>
  ...
  let ranges = $derived([
    ...
    { label: "direct", pwmMin: pwmOff, pwmMax: pwmHandover },
    { label: "governed", pwmMin: pwmHandover, pwmMax: throttleHigh },
...
<div class="container">
  <div class="meter-wrapper">
    ...
    <div class="current-text" style:left={`${markerOffset}%`}>
      {#if current < pwmOff}
        OFF
      {:else if current < pwmHandover}
        {currentPercent.toFixed(1)}% Direct
      {:else}
        {currentPercent.toFixed(1)}% Target
      {/if}
    </div>
