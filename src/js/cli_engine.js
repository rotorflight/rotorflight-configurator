...
export default class CliEngine {
    ...
    get outputHistory() { return this.#outputHistory; }
    clearOutputHistory() {
        this.#outputHistory = "";
        this.#GUI.windowWrapper.empty();
    }

    get errorsCount() { return this.#cliErrorsCount; }
    get inBatchMode() { return this.#inBatchMode; }
    ...
    initializeAutoComplete() {
        const self = this;
        CliAutoComplete.initialize(this.#GUI.textarea, this.sendLine, this.writeToOutput);
        ...
        this.#GUI.textarea.keydown(event => {
            if (event.key === 'Tab') {
                ...
                if (this.#cliAutoComplete) {
                    ...
                    else if (!this.#cliAutoComplete.isOpen() && !this.#cliAutoComplete.isBuilding()) {
    ...
    async executeCommandsArray(commandsArray) {
        ...
        for (let i = 0, delay = 0; i < commandsArray.length; i++) {
            ...
            delay = line.toLowerCase().startsWith('profile') ? this.#profileSwitchDelayMs : 0;
            delay = line.toLowerCase().startsWith('rateprofile') ? this.#rateProfileSwitchDelayMs : 0;
    ...
    readSerial(readInfo) {
        ...
        */
        ...
        for (const charCode of data) {
            ...
            const isCRLF = currentChar.charCodeAt() === CHAR_CODE_LINE_FEED || currentChar.charCodeAt() === CHAR_CODE_CARRIAGE_RETURN;

            if (!CONFIGURATOR.cliEngineValid && (isCRLF || this.#startProcessingForValidation)) {
            ...
            if (this.#cliBuffer === 'Command batch started' && CHAR_CODE_BACKSPACE !== charCode) {
                this.#inBatchMode = true;
            }

            if (this.#cliBuffer === 'Command batch ended' && CHAR_CODE_BACKSPACE !== charCode) {
        ...
        if (!CONFIGURATOR.cliEngineValid && validateText.indexOf('CLI') !== -1) {
            ...
            if (this.#cliAutoComplete && this.#cliAutoComplete.isEnabled() && !this.#cliAutoComplete.isBuilding()) {
        ...
        if (CONFIGURATOR.cliEngineValid) {
            ...
            if (((this.#cliAutoComplete && !this.#cliAutoComplete.isEnabled()) || this.#cliAutoComplete == undefined) && this.#cliBufferContainsPartialCommand) {
