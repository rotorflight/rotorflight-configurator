...
export default class PresetTracker {
  ...
  #prune() {
    this.#trackedPresets.splice(
      MAX_TRACKED_PRESETS_COUNT + 1,
      this.#trackedPresets.length,
    );
  }
   ...
   */
  remove(viewURL) {
    const index = this.#trackedPresets.findIndex(
      (trackedPreset) => trackedPreset.viewURL === viewURL,
    );
  ...
  find(viewURL) {
    return this.#trackedPresets.find(
      (trackedPreset) => trackedPreset.viewURL === viewURL,
    );
  }
}
