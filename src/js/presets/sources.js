...
export default class Sources {
    ...
    #saveSourcesMetadataToStorage() {
        ConfigStorage.set({'PresetsSourcesMetadata': this.#collectSourcesMetadata()});
    }
    ...
    #officialSourceMetadata() {
        const officialSource = new Metadata("Rotorflight Official Presets", "https://github.com/Oats87/rotorflight-presets", "main");
    ...
    async #newSource(metadata, selected = false) {
        const source = new Source(this.#dom.divSourcesPanel, metadata, `source_panel_${this.#panelCounter}`);
        this.#panelCounter++;
        
        if(!metadata.official) {
            source.sourceChangedCallback = this.#saveSourcesMetadataToStorage.bind(this);
        }
    ...
    #onAddNewSourceButtonClick() {
        const presetSource = new Metadata(i18n.getMessage("presetsSourcesDialogDefaultSourceName"), "", "");
