...
export default class Retriever {
    ...
    async retrieveIndex() {
        try {
            const res = await fetch(`${this.#rawUrl}index.json`, { cache: "no-cache" });
                ...
                promises.push(promise.then(text => {
                    ...
                    for (let line of tmpStrings) {
                        if (!line.startsWith(this.#index.settings.MetapropertyDirective) || 
                            line.slice(this.#index.settings.MetapropertyDirective.length).trim().toLowerCase().startsWith(this.#index.settings.OptionsDirectives.OPTION_DIRECTIVE)) {
                            strings.push(line);
                        }
                    }
                    return strings;
    ...
    async loadPreset(presetInstance) {
        const presetText = await this.#fetchFile(this.#rawUrl + presetInstance.presetData.fullPath);
    ...
    async #loadPresetWarning(presetInstance) {
        ...
        if (presetInstance.presetData.disclaimer) {
            presetInstance.completeWarning += (presetInstance.completeWarning ? "\n" : "") + presetInstance.presetData.disclaimer;
        }

        const includedWarningAndDisclaimerFiles = [].concat(...[presetInstance.presetData.include_warning, presetInstance.presetData.include_disclaimer].filter(Array.isArray));

        const includedWarningsAndDisclaimers = await this.#fetchAllFiles(includedWarningAndDisclaimerFiles);
...
class Parser {
    ...
    #parseProperty(presetInstance, line, propertyName) {
        switch (this.#settings.presetsFileMetadata[propertyName].type) {
            ...
            case this.#settings.MetadataTypes.PARSER:
                ...
                break;
            default:
                console.err(`Parsing preset: unknown property type '${this.#settings.presetsFileMetadata[propertyName].type}' for the property '${propertyName}'`);
        }
    }
    ...
    #parseOptionDirective(presetInstance, line) {
        ...
        if (this.#isOptionBegin(line)) {
            ...
            const optionGroup = new OptionGroup(this.#getOptionGroupName(line), [], this.#isExclusiveOptionGroup(line));
